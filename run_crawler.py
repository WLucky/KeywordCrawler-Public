...
def update_config(platform, keywords, time_type=0, max_notes=15, max_comments=10, enable_sub_comments=False, enable_tavily_img=False, max_images_per_result=3, enable_video_download=True):
    ...
    if platform == 'dy':
        ...
        # 更新SEARCH_COUNTRY（默认为"china"表示中国）
        tavily_content = re.sub(r'SEARCH_COUNTRY = ["\'][^"\']*["\']', 'SEARCH_COUNTRY = "china"', tavily_content)

        with open(tavily_config_file, 'w', encoding='utf-8') as f:
            f.write(tavily_content)

        print(f'已更新Tavily时间范围为: {tavily_time_range}')
        print(f'已更新Tavily搜索国家为: china')
        if time_type > 0:
    ...
    for platform in platforms:
        ...
        if platform_dir.exists():
            for ext in ['json', 'jsonl', 'csv', 'xlsx']:
                ...
                content_files.extend(list(platform_dir.glob(f'**/*{platform_full_names.get(platform, platform)}*.{ext}')))
